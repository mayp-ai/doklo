...
function Flow({ detail, sourceRepository }: { detail: DokDetailProjection; sourceRepository: SourceRepository | null }) {
  ...
  return <section className="mt-8" aria-labelledby="flow-title" data-testid="dok-detail-flow">
    ...
    <ol className="m-0 list-none space-y-3 p-0">{detail.steps.map((step, index) => {
      if (step.actorKind === 'system' && !showSystem) return null;
      const displayNumber = step.actorKind === 'system' ? null : ++userIndex;
      return <li key={`${step.order}-${index}`} className={`py-3 ${step.actorKind === 'system' ? 'ml-10 rounded-md border border-border bg-accent-soft px-4' : ''}`}>
        <div className="flex items-start gap-3">{displayNumber === null ? <span className="grid h-7 w-7 flex-none place-items-center rounded-md bg-accent-soft-strong text-accent"><Icon kind="system"/></span> : <span className="grid h-7 w-7 flex-none place-items-center rounded-full border border-border bg-surface-2 font-mono text-xs font-semibold text-ink-secondary">{String(displayNumber).padStart(2, '0')}</span>}<div className="min-w-0 flex-1">
